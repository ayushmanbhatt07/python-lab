...
def generate_magic_square(n):
    if n % 2 == 1:  # Odd order magic square
    ...
    else:  # Singly even order magic square
        half_n = n // 2
        ...
        for i in range(half_n):
            for j in range(k):
                ...
                magic_square[i][j + half_n - k], magic_square[i + half_n][j + half_n - k] = magic_square[i + half_n][j + half_n - k], magic_square[i][j + half_n - k]
        for i in range(k):
            for j in range(half_n):
                magic_square[i][j], magic_square[i + half_n][j] = magic_square[i + half_n][j], magic_square[i][j]
    return magic_square
